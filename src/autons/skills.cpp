...
void skills75() {
    ...
    chassis.pid_wait();
    chassis.pid_turn_set(90_deg, 100);//turning infront of matchloader
    chassis.pid_wait();
    chassis.pid_odom_set({{-62_in, -47_in},  rev, 67}); //at the matchloader
    chassis.pid_wait();
    pros::delay(1670);//matchloading
    ...
    chassis.pid_wait();
    chassis.pid_odom_set({{46_in, -33_in}, fwd, 130}); //change this to 41 instead of 46
    chassis.pid_wait(); //crossing the bottom long goal
    
    bunny.set_value(0);
    chassis.pid_turn_set(0_deg, 100); 
    chassis.pid_wait();
    chassis.pid_odom_set({{46_in, -47_in}, rev, 110}); //change to 41
    chassis.pid_wait();
    chassis.pid_turn_set(270_deg, 100);
    chassis.pid_wait();
    chassis.pid_odom_set({{21_in, -47_in}, fwd, 110}); 
    chassis.pid_wait();
    highskills();
    loader.set_value(1);
